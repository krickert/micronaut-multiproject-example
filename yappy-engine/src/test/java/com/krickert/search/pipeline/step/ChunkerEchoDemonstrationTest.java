...
public class ChunkerEchoDemonstrationTest {
    ...
    void setUp() {
        ...
        // Set up mock chunker client
        when(chunkerClient.processData(any(ProcessRequest.class))).thenAnswer(invocation -> {
        ...
        // Set up mock echo client
        when(echoClient.processData(any(ProcessRequest.class))).thenAnswer(invocation -> {
            ProcessRequest request = invocation.getArgument(0);
