...
@Singleton
@Requires(beans = {PipelineConfigService.class, PipelineServiceImpl.class, KafkaSerdeProvider.class, ObjectMapper.class},
...
public class DynamicKafkaConsumerManager implements ApplicationEventListener<RefreshEvent> {
    ...
    public synchronized void synchronizeConsumers() {
        ...
        Map<String, PipelineConfig> allConfigs = configService.getPipelineConfigs();
        if (allConfigs == null || allConfigs.isEmpty()) {
            log.warn("No pipeline configurations found. Stopping all consumers.");
            allConfigs = Map.of();
        }

        final Map<String, PipelineConfig> currentConfigs = allConfigs;
