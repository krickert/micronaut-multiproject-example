...
This document is Part I of four parts in an immersive tutorial which builds an enterprise fully-open-source search indexing pipeline and
search engine that scales with both indexing and searching your document corpus.
...
this tutorial. It's open and used to demonstrate how one can build a multi-module build
 for containers in a single repository using the Micronaut framework.  At the same time, this tutorial will build a kafka/grpc-based text
 processing pipeline.
...
nano protobuf-models/src/test/java/com/krickert/search/model/ProtobufUtilsTest.java
----
+
++++
<details>
