...
public class ValueHandler {
     ...
     private Object convertValue(Object sourceValue, FieldDescriptor targetField, String ruleForError,
        ...
        if (targetIsMap) {
            if (allowListOrMapSource && sourceIsMap) return sourceValue;
                 ...
                 FieldDescriptor valueDesc = mapEntryDesc.findFieldByName("value");
                 if (valueDesc == null) throw new MappingException("Map entry descriptor missing value field: " + mapEntryDesc.getFullName(), ruleForError);
     ...
     private Object convertSingleValue(Object sourceValue,
        ...
        if (sourceJavaType == targetJavaType) {
            if (sourceJavaType == JavaType.MESSAGE) {
                if (targetMessageDesc == null) throw new IllegalStateException("Internal Error: Target MessageDescriptor is null for MESSAGE type field " + targetFieldName);
                ...
                if (!sourceDesc.getFullName().equals(targetMessageDesc.getFullName())) {
                     if (targetMessageDesc.getFullName().equals(Value.getDescriptor().getFullName())) {
                          if (sourceValue instanceof Struct || sourceValue instanceof Struct.Builder ||
                              ...
                              sourceValue instanceof Value) {
                               try { return wrapValue(sourceValue); }
                               catch (MappingException e) { throw new MappingException(String.format("Failed to wrap WKT %s into Value for field '%s'", sourceDesc.getName(), targetFieldName) , e, ruleForError); }
                          }
                     }
                 ...
                 if (sourceValue instanceof Message.Builder) {
                      try { return ((Message.Builder)sourceValue).build(); }
                      catch(UninitializedMessageException e) { throw new MappingException("Source message builder is uninitialized for target field '" + targetFieldName + "'", e, ruleForError); }
                 }
            }
            if (sourceJavaType == JavaType.ENUM) {
                 if (targetEnumDesc == null) throw new IllegalStateException("Internal Error: Target EnumDescriptor is null for ENUM type field " + targetFieldName);
        ...
        try {
            if (targetJavaType == JavaType.MESSAGE && targetMessageDesc != null &&
                targetMessageDesc.getFullName().equals(Value.getDescriptor().getFullName())) {
                 try { return wrapValue(sourceValue); }
                 catch (MappingException e) { throw new MappingException(String.format("Failed to wrap source type %s into Value for field '%s'", sourceJavaType, targetFieldName), e, ruleForError); }
            }
            switch (targetJavaType) {
                ...
                case ENUM:
                    ...
                    if (sourceValue instanceof Number) {
                        ...
                        if (enumValue == null) throw new MappingException("Invalid enum number " + num + " for enum type " + targetEnumDesc.getFullName(), null, ruleForError);
                        return enumValue;
                    }
                    if (sourceJavaType == JavaType.STRING) {
                        EnumValueDescriptor enumValue = targetEnumDesc.findValueByName((String) sourceValue);
                        if (enumValue == null) throw new MappingException("Invalid enum name '" + sourceValue + "' for enum type " + targetEnumDesc.getFullName(), null, ruleForError);
                        return enumValue;
                    }
                    break;
                case BOOLEAN:
                ...
                case BYTE_STRING:
                    ...
                    break;
                case MESSAGE: break; // Handled by exact type match or Value wrapping
            }
         ...
         if (value == null) valueBuilder.setNullValue(NullValue.NULL_VALUE);
             ...
             valueBuilder.setListValue(listBuilder);
              ...
              for (Map.Entry<?, ?> entry : ((Map<?, ?>) value).entrySet()) {
                   if (!(entry.getKey() instanceof String)) throw new MappingException("Cannot wrap Map with non-String keys into Struct Value", null, null);
                   structBuilder.putFields((String) entry.getKey(), wrapValue(entry.getValue()));
              }
              valueBuilder.setStructValue(structBuilder);
         }
         else if (value instanceof MessageOrBuilder) throw new MappingException("Cannot automatically wrap Message type " + ((MessageOrBuilder)value).getDescriptorForType().getFullName() + " into Protobuf Value.", null, null);
         else if (value instanceof EnumValueDescriptor) valueBuilder.setStringValue(((EnumValueDescriptor)value).getName());
         else if (value instanceof ByteString) valueBuilder.setStringValue(Base64.getEncoder().encodeToString(((ByteString)value).toByteArray()));
         else throw new MappingException("Cannot wrap unsupported Java type " + value.getClass().getName() + " into Protobuf Value", null, null);
     ...
     private Object unwrapValue(Value valueProto) {
         ...
         switch (valueProto.getKindCase()) {
             case NUMBER_VALUE: return valueProto.getNumberValue();
             case STRING_VALUE: return valueProto.getStringValue();
             case BOOL_VALUE: return valueProto.getBoolValue();
             case STRUCT_VALUE: return valueProto.getStructValue();
             case LIST_VALUE: return valueProto.getListValue().getValuesList().stream().map(this::unwrapValue).collect(Collectors.toList());
             case NULL_VALUE: default: return null;
         }
     }

     private Object getDefaultForNullSource(FieldDescriptor targetField) {
         switch (targetField.getJavaType()) {
             case INT: return 0; case LONG: return 0L; case FLOAT: return 0.0f; case DOUBLE: return 0.0d;
             case BOOLEAN: return false; case STRING: return ""; case BYTE_STRING: return ByteString.EMPTY;
             case ENUM: return targetField.getEnumType().getValues().get(0);
             case MESSAGE:
     ...
     Struct.Builder getStructBuilder(Message.Builder parentBuilder, FieldDescriptor structField, String ruleForError) throws MappingException {
          if (structField.getJavaType() != JavaType.MESSAGE || !structField.getMessageType().getFullName().equals(Struct.getDescriptor().getFullName())) throw new IllegalArgumentException("Field is not a Struct field: " + structField.getFullName());
         try {
             ...
             else if (currentValue instanceof Message && ((Message)currentValue).getDescriptorForType().equals(Struct.getDescriptor())) structBuilder.mergeFrom((Message)currentValue);
             else if (currentValue != null && !currentValue.equals(Struct.getDefaultInstance())) throw new MappingException("Field '" + structField.getName() + "' exists but is not a Struct: " + currentValue.getClass().getName(), null, ruleForError);
     ...
     private List<Message> convertJavaMapToMapFieldList(Map<?, ?> javaMap, FieldDescriptor mapField) throws MappingException {
         ...
         if (keyDesc == null || valueDesc == null) throw new MappingException("Map entry descriptor missing key or value field: " + entryDesc.getFullName());
     ...
     private Map<Object, Object> convertMapFieldListToJavaMap(Object mapFieldValue, FieldDescriptor mapField) throws MappingException {
         ...
         for (Object entryObj : entries) {
             if (!(entryObj instanceof Message entryMsg)) throw new MappingException("Map entry in field '" + mapField.getName() + "' was not a Message during reconstruction.");
             if (!entryMsg.getDescriptorForType().equals(entryDesc)) throw new MappingException("Map entry message has incorrect type: expected " + entryDesc.getFullName() + ", got " + entryMsg.getDescriptorForType().getFullName());
     ...
     private Map<Object, Object> convertAndTypeCheckJavaMap(Map<?, ?> sourceMapRaw, FieldDescriptor mapField, String ruleForError) throws MappingException {
          ...
          if (keyDesc == null || valueDesc == null) throw new MappingException("Map entry descriptor missing key or value field: " + mapEntryDesc.getFullName(), ruleForError);
