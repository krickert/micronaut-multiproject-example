 ...
 * 2. Handles null/blank topics and services
...
class WhitelistValidatorTest {
    ...
    void validate_nonWhitelistedKafkaListenTopic_returnsError() {
        ...
        assertTrue(errors.stream().anyMatch(e -> e.contains("listens to non-whitelisted Kafka topic")), 
    ...
    void validate_nonWhitelistedKafkaPublishTopic_returnsError() {
        ...
        assertTrue(errors.stream().anyMatch(e -> e.contains("publishes to non-whitelisted Kafka topic")), 
    ...
    void validate_nonWhitelistedGrpcForwardTo_returnsError() {
        ...
        assertTrue(errors.stream().anyMatch(e -> e.contains("forwards to non-whitelisted gRPC service")), 
    ...
    void validate_stepHasNoKafkaOrGrpc_returnsNoErrors() {
        // Create a pipeline configuration with no Kafka topics or gRPC services
        Set<String> allowedKafkaTopics = new HashSet<>(Arrays.asList("topic1", "topic2"));
        Set<String> allowedGrpcServices = new HashSet<>(Arrays.asList("service1", "service2"));

        PipelineClusterConfig clusterConfig = createTestClusterConfig(
            allowedKafkaTopics, allowedGrpcServices, null, null, null
        );

        List<String> errors = validator.validate(clusterConfig, schemaContentProvider);

        assertTrue(errors.isEmpty(), "Configuration with no Kafka topics or gRPC services should not produce errors");
    }
    ...
    // Note: We can't directly test null/blank values in the lists because the PipelineStepConfig record constructor
    // validates the inputs and throws exceptions for null elements. The WhitelistValidator's null/blank checks
    // are still important for robustness, but they're effectively redundant with the record constructor's validation.
    ...
    // Note: We removed the createTestClusterConfigWithNullOrBlankTopic method because we can't bypass the record constructor's validation
}
