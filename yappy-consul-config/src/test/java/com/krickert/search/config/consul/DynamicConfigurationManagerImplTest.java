...
import jakarta.inject.Inject; // Ensure this is jakarta.inject
...
import java.util.HashSet; // For creating mutable sets
import java.util.List;
import java.util.Map;
import java.util.Optional;
import java.util.Set; // For type hinting
...
class DynamicConfigurationManagerImplTest {
    ...
    // Using a real ObjectMapper for calculateExpectedVersion,
    // but keeping the mock for verifying interactions if other tests need it.
    // If no other test verifies mockObjectMapper, it can be removed.
    @Mock
    private ObjectMapper mockObjectMapper;

    @Inject // Inject the real ObjectMapper for use in helper
    ...
    @BeforeEach
    void setUp() {
        // Pass the mockObjectMapper to the factory if it's what the SUT expects for its internal operations
        // If SUT needs a real one, then inject and pass the real one.
        // Based on SUT constructor, it takes an ObjectMapper.
        // For tests where SUT's objectMapper usage is not under test, passing the mock is fine.
        // For generateConfigVersion, it uses its own real one or the injected real one.
        dynamicConfigurationManager = TestDynamicConfigurationManagerFactory.createDynamicConfigurationManager(
                ...
                mockEventPublisher,
                mockConsulKvService, // This mock is used by the factory
                mockConsulBusinessOperationsService, // This mock is used by the factory
                new ObjectMapper()// Pass the mock ObjectMapper to SUT if its internal serialization is mocked/verified
                // If SUT needs a real ObjectMapper for its internal logic (like generateConfigVersion),
                // then you should inject and pass the real one here.
                // The SUT's generateConfigVersion uses its own objectMapper field.
        );
    }
    ...
    // Helper to create a new config with modified topics
    ...
    void handleConsulClusterConfigUpdate_successfulUpdate_validatesAndCachesAndPublishes() {
        ...
        PipelineClusterConfig newWatchedConfig = createTestClusterConfig(TEST_CLUSTER_NAME, moduleMapNew); // Use helper
    ...
    void publishEvent_directListenerThrows_continuesNotifyingOtherListenersAndMicronautPublisher() {
        ...
        verify(misbehavingListener).accept(eventCaptor.capture());
        ...
        verify(wellBehavedListener).accept(eventCaptor.capture());
    ...
    void initialize_successfulLoad_isNotStaleAndVersionIsSet() {
        ...
        assertFalse(versionOpt.get().equals("null-config"), "Version should not be 'null-config'");
    ...
    // Modify calculateExpectedVersion to use a new ObjectMapper instance
    private String calculateExpectedVersion(PipelineClusterConfig config) {
        try {
            ObjectMapper localObjectMapper = new ObjectMapper(); // Create and use a local, real ObjectMapper
            String jsonConfig = localObjectMapper.writeValueAsString(config);
    ...
    // Test State After Watch Update Fails Validation (Keeps Old State)
    @Test
    ...
    // Test State After Watch Deletion Event
    @Test
    ...
    // Test State After Watch Error Event
    @Test
