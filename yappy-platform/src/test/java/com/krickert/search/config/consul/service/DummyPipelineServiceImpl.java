...
public class DummyPipelineServiceImpl extends PipeStreamEngineGrpc.PipeStreamEngineImplBase {
    ...
    /**
     * Processes a PipeStream and returns a PipeStream.
    ...
    public void process(PipeStream request, StreamObserver<PipeStream> responseObserver) {
        ...
        LOG.info("Document ID: {}",
                request.hasCurrentDoc() ? request.getCurrentDoc().getId() : "null");

        // Create a processed PipeDoc with realistic data
        PipeDoc processedDoc;
        if (request.hasCurrentDoc()) {
            // Use the input document as a base and enhance it
            processedDoc = enhancePipeDoc(request.getCurrentDoc());
        } else {
        ...
        // Use the input stream as a base
        finalStreamBuilder.mergeFrom(request);

        // Set the current document to our processed document
        finalStreamBuilder.setCurrentDoc(processedDoc);

        // Increment the hop number
        finalStreamBuilder.setCurrentHopNumber(request.getCurrentHopNumber() + 1);

        // Send the response
        responseObserver.onNext(finalStreamBuilder.build());
        responseObserver.onCompleted();
    }

    /**
     * Processes a PipeStream asynchronously and returns an Empty response.
